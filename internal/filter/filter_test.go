...
func TestDecideAppliesAllowBeforeBlock(t *testing.T) {
	...
	got := rs.Decide(domain(t, "news.doubleclick.net"), "", noAddress, testNow)
...
func TestDecideSubdomainRuleMatchesTheDomainItself(t *testing.T) {
	...
	got := rs.Decide(domain(t, "doubleclick.net"), "", noAddress, testNow)
...
func TestDecideExactRuleDoesNotMatchChildName(t *testing.T) {
	...
	require.Equal(t, filter.ActionAllow, rs.Decide(domain(t, "cdn.ads.example.com"), "", noAddress, testNow).Action)
	require.Nil(t, rs.Decide(domain(t, "cdn.ads.example.com"), "", noAddress, testNow).Match)
	require.Equal(t, filter.ActionBlock, rs.Decide(domain(t, "ads.example.com"), "", noAddress, testNow).Action)
}

func TestDecideWithoutAMatchAllows(t *testing.T) {
	rs := compile(t)

	got := rs.Decide(domain(t, "example.com"), "", noAddress, testNow)
...
func TestDecidePrefersTheMoreSpecificRule(t *testing.T) {
	...
	got := rs.Decide(domain(t, "x.ads.doubleclick.net"), "", noAddress, testNow)
...
func TestDecidePrefersExactOverSubdomain(t *testing.T) {
	...
	got := rs.Decide(domain(t, "ads.example.com"), "", noAddress, testNow)
...
func TestDecideKeepsAllowFromOneSourceWhenAnotherBlocks(t *testing.T) {
	...
	got := rs.Decide(domain(t, "www.example.com"), "", noAddress, testNow)
