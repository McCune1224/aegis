...
func TestCompileResolvesAProfileChain(t *testing.T) {
	...
	got := set.Decide(mustParse("example.com"), "", noAddress, testNow)
...
func TestAClientGetsThePolicyOfItsOwnProfile(t *testing.T) {
	...
	require.Equal(t, filter.Refused, set.Decide(mustParse("example.com"), "tablet", noAddress, testNow).Policy.Mode)
	require.Equal(t, filter.NullAddress, set.Decide(mustParse("example.com"), "laptop", noAddress, testNow).Policy.Mode)
	require.Equal(t, filter.NullAddress, set.Decide(mustParse("example.com"), "", noAddress, testNow).Policy.Mode)
}
